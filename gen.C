...
void gen(int option = 0, int nevents = 1e5){
	...
	TVector3 b3v, t3v, e3v, q3v, d3v, u3v, boostcm, boostcms, e3vs, qv3s, d3vs;
	...
	for(int i = 0; i < nevents; i++){
		...
		emin = 0;
		emax = (pow(Wcut_max,2) - mt*mt - 2.*beamE*mt)/2./(beamE*cos(the) - beamE - mt);
		emin = (pow(Wcut_min,2) - mt*mt - 2.*beamE*mt)/2./(beamE*cos(the) - beamE - mt);
		...
		if(d4v.Theta()*TMath::RadToDeg() < th_min_det || d4v.Theta()*TMath::RadToDeg() > th_max_det){
			i--;
			continue;
		}
